...
def publish_video_roi_preview(client, video_name):
    ...
    print(f"[EDGE] Da gui preview ROI cho video: {video_name}")
...
def ai_processing_loop(client):
    ...
    if current_mode == "video" and stop_reason == "reset_roi":
        print("[EDGE] Da dung xu ly video de reset ROI, bo qua goi Complete.")
    stop_reason = None
        ...
        try:
            ...
            if action in ("update_roi", "update_zones"):
                ...
                if roi_pts and len(roi_pts) == 4:
                    ...
                    roi_confirmed = True
                    print(f"[EDGE] Da cap nhat ROI moi: {roi_pts}")
            ...
            if action == "reset_roi":
                ...
                print("[EDGE] Da dung xu ly, xoa buffer ROI cu va reset ROI ve mac dinh.")
                return
            ...
            if action == "start":
                if not is_running:
                    ...
                    if current_mode == "video" and not cmd.get("roi"):
                        print("[EDGE] Start video khong co ROI da luu -> chi gui preview, khong xu ly.")
                        publish_video_roi_preview(client, active_video)
                        return
                    ...
                    lane_detector.set_roi(roi_pts)
                    roi_confirmed = True
                    print(f"[EDGE] Da set ROI truoc khi xu ly: {roi_pts}")
                    ...
                    print("[EDGE] Luong AI dang chay roi!")

            elif action == "stop":
                is_running = False
                stop_reason = "stop"
                print("[EDGE] Dang dung xu ly...")
